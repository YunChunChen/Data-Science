...
__host__ __device__ int NumberOfSetBitsGPU(int i);
__global__ static void Support(int* a, int* b, int* temp, int* support, int width, int length);
...
__global__ static void Support(int* a, int* b, int* temp, int* result, int width, int length) {
    
    __shared__ static int shared[BLOCKNUM][THREADNUM];
    const int tid = threadIdx.x;
    const int bid = blockIdx.x;

    for (int i = 0 ; i <= width/BLOCKNUM; i++) {
        if (bid + i*BLOCKNUM >= width)
            break;
        shared[bid][tid] = 0;
        for (int j = tid; j < length; j += THREADNUM) {
            temp[j+(bid + i*BLOCKNUM)*length] = a[j] & b[j+(bid + i*BLOCKNUM)*length];
            shared[bid][tid] += NumberOfSetBitsGPU(temp[j+(bid + i*BLOCKNUM)*length]);
        }
        __syncthreads();
        if (tid == 0) {
            for (int j = 1; j < THREADNUM; j++)
                shared[bid][0] += shared[bid][j];
            result[bid + i*BLOCKNUM] = shared[bid][0];
        }
    }
}
...
void mineGPU(EClass *eClass, int minSup, int* index, int length){
    ...
    for (int i = 0 ; i < size; i++) {
        ...
        int* tmp;
        int* supp;
        int array_length = sizeof(int)*length;
        int array_width = size-i+1;

        cudaMalloc(&a_array, array_length);
        cudaMemcpy(a_array, a, array_length, cudaMemcpyHostToDevice);

        int* tmp_b = new int [array_length*array_width];
        for (int j = i+1; j < size; j++)
            memcpy(tmp_b + (j-i-1)*length, eClass->items[j].db, array_length);

        cudaMalloc(&b_array, array_length*array_width);
        cudaMalloc(&tmp, array_length*array_width);
        cudaMalloc(&supp, sizeof(int)*array_width);
        cudaMemcpy(b_array, tmp_b, array_length*array_width, cudaMemcpyHostToDevice);
        
        free(tmp_b);
        Support<<<BLOCKNUM, THREADNUM>>> (a_array, b_array, tmp, supp, array_width, length);

        int* support = new int [sizeof(int)*array_width];
        cudaMemcpy(support, supp, sizeof(int)*array_width, cudaMemcpyDeviceToHost);
        int* temp = new int [array_length*array_width];
        cudaMemcpy(temp, tmp, array_length*array_width, cudaMemcpyDeviceToHost);
        ...
        for (int j=i+1; j < size; j++) {
            int support_val = support[j-i-1];
            if (support_val >= minSup) {
                int* tmp = new int [length];
                memcpy(tmp, temp+(j-i-1)*length, array_length);
                children->items.push_back(Item(eClass->items[j].id, tmp, support_val));
            }
        }
        free(support);
        free(temp);
...
__host__ __device__ int NumberOfSetBitsGPU(int i) {
        int ans;
        ans = i - ((i >> 1) & 0x55555555);
        ans = (ans & 0x33333333) + ((ans >> 2) & 0x33333333);
        return (((ans + (ans >> 4)) & 0x0F0F0F0F) * 0x01010101) >> 24;
}
